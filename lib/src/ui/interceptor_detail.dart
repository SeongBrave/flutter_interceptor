    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: TabBarView(
          children: [
            ContentDetialPage(
                httpTransaction: widget.httpTransaction,
                page: DetailPage.Content),
            ContentDetialPage(
                httpTransaction: widget.httpTransaction,
                page: DetailPage.Request),
            ContentDetialPage(
                httpTransaction: widget.httpTransaction,
                page: DetailPage.Resopnse),
          ],
        ),
      ),
    );
...
enum DetailPage { Request, Resopnse, Content }
  ...
  Widget build(BuildContext context) {
    ...
    switch (widget.page) {
            ...
            children: [
              Row(
                children: [
                  ...
                  Expanded(
                      child: Text(
                    "${widget.httpTransaction.muri.toString()}",
              ...
              Row(
                children: [
                  Text("${local.method!}:"),
                  Text("${widget.httpTransaction.method}")
                ],
              ),
              Row(
                children: [
                  Text("${local.requestTime!}:"),
                  Text("${widget.httpTransaction.requestTime}")
                ],
              ),
              Row(
                children: [
                  Text("${local.responseTime!}:"),
                  Text("${widget.httpTransaction.responseTime}")
                ],
              ),
              Row(
                children: [
                  Text("${local.duration!}:"),
                  Text("${widget.httpTransaction.duration}ms")
                ],
              )
            ],
          ),
      ...
      case DetailPage.Request:
        ...
        widget.httpTransaction.requestHeaders?.forEach((key, value) {
          list.add(Row(
            children: [
              Text("$key:"),
              Expanded(child: Text(convert.jsonEncode(value)))
            ],
        ...
        list.add(Padding(
            ...
            child: Text(widget.httpTransaction.requestBody ?? "")));
        return SingleChildScrollView(
          child: Column(
            children: list,
          ),
      ...
      case DetailPage.Resopnse:
        ...
        list.add(Padding(
          padding: EdgeInsets.only(top: 25),
          child: Text(json(widget.httpTransaction.responseBody)),
        ));
        return SingleChildScrollView(
          child: Column(
            children: list,
